...
#include <sys/stat.h>
#include <limits.h>
#include "/home/qiuyu/code/Linux-Kernel-Analysis/path_alloc/path_alloc.h"
...
static int dopath(Myfunc *func)     /* we return whatever func() returns */
{
    ...
    DIR *dp;
    int ret, n;
    if (lstat(fullpath, &statbuf) == -1)    /* stat error */
    ...
    if ( (ret = func(fullpath, &statbuf, FTW_D)) != 0)
        return ret;
    n = strlen(fullpath);
    if (n + NAME_MAX + 2 > (int)pathlen) {   /* expand path buffer */
        pathlen *= 2;
        if ( (fullpath = (char *)realloc(fullpath, pathlen)) == NULL)
            err_sys("realloc failed");
    }
